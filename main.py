...
if __name__ == "__main__":
    ...
    FCM.FCM(ImgTraining, HodTraining, t, c, m, eps)
    folderU = 'U'
    folderV = 'V'
    ...
    for fileName in sorted(os.listdir(os.path.join('Data','Testing'))):
        ...
        #img=cv2.imread(os.path.join('Data',os.path.join('Testing',...)),0)
            ...
            cv2.imwrite(os.path.join(args['output'],fileName),O_img)
            #cv2.imwrite('ver1_'+fileName,O_img)
    result = RMSE.Accuracy(os.path.join('Data','Testing'), args['output'])
    np.savetxt(args['accuracy'], result, fmt = '%.4f', delimiter = ',')
    end_time = time.time()
